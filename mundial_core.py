...
def calcular_stats_globales(matches: list[dict]) -> dict:
    """Estadísticas acumuladas de TODOS los partidos jugados (cualquier fase),
...
# ─────────────────────────────────────────────────────────────────────────
# BONUS POR RONDA Y PUNTUACIÓN TOTAL
# ─────────────────────────────────────────────────────────────────────────
...
def calcular_bonus_ronda(matches: list[dict]) -> dict:
    bonus = {eq: 0 for eq in TODOS_LOS_EQUIPOS}
    for fase, puntos in PUNTOS_BONUS_RONDA.items():
        for eq in equipos_que_jugaron_fase(matches, fase):
            bonus[eq] = bonus.get(eq, 0) + puntos
    return bonus


def calcular_bonus_grupo(clasificaciones: dict) -> dict:
    bonus = {}
    for letra, tabla in clasificaciones.items():
        for fila in tabla:
            bonus[fila["equipo"]] = PUNTOS_POS_GRUPO.get(fila["pos"], 0)
    return bonus


def calcular_puntos_totales(matches: list[dict], manual: dict) -> dict:
    """Devuelve, por equipo: el desglose completo de puntos + el total."""
    stats = calcular_stats_globales(matches)
    clasificaciones = {letra: clasificar_grupo(letra, matches, stats) for letra in GRUPOS}
    bonus_grupo = calcular_bonus_grupo(clasificaciones)
    ...
    return resultado, clasificaciones
...
def generar_data_json(matches: list[dict], manual: dict) -> dict:
    puntos, clasificaciones = calcular_puntos_totales(matches, manual)
...
def construir_y_guardar(matches_path: Path, manual_path: Path, salida_path: Path) -> dict:
    ...
    manual = cargar_json(manual_path, {"premios_finales": {}, "goleador_equipo": None})
